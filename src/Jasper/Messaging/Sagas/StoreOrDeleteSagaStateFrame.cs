...
namespace Jasper.Messaging.Sagas
{
    public class StoreOrDeleteSagaStateFrame : SyncFrame
    {
        ...
        public StoreOrDeleteSagaStateFrame(Variable document, Type sagaHandlerType)
        {
            ...
            if (prop == null) throw new ArgumentOutOfRangeException(nameof(document), $"Saga state document type {document.VariableType.FullNameInCode()} does not have an Id property that is required for usage with the in memory saga persistence");
        }
        ...
        public override void GenerateCode(GeneratedMethod method, ISourceWriter writer)
        {
            ...
            writer.Write($"{_persistor.Usage}.{nameof(InMemorySagaPersistor.Delete)}<{_document.VariableType.FullNameInCode()}>({_document.Usage}.Id);");
            writer.FinishBlock();
