...
namespace Jasper.Persistence.SqlServer.Resiliency
{
    public class RunScheduledJobs : IMessagingAction
    {
        ...
        public RunScheduledJobs(IWorkerQueue workers, SqlServerSettings mssqlSettings, ITransportLogger logger,
            IRetries retries, JasperOptions settings)
        {
            _workers = workers;
        ...
        {
            if (!await conn.TryGetGlobalLock(ScheduledJobLockId)) return null;
