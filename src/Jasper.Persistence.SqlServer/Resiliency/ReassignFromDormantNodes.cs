...
namespace Jasper.Persistence.SqlServer.Resiliency
{
    public class ReassignFromDormantNodes : IMessagingAction
    {
        ...
        public ReassignFromDormantNodes(SqlServerSettings marker, JasperOptions settings)
        {
            _settings = settings;
            _fetchOwnersSql = $@"
...
";
        ...
        private async Task<List<int>> fetchOwners(SqlTransaction tx)
        {
            ...
            using (var reader = await tx.Connection.CreateCommand(tx, _fetchOwnersSql)
                .With("owner", _settings.UniqueNodeId).ExecuteReaderAsync())
            {
