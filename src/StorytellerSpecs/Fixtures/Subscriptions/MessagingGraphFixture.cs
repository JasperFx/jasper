...
namespace StorytellerSpecs.Fixtures.Subscriptions
{
    ...
    public class MessagingGraphFixture : BusFixture
    {
        ...
        {
            return VerifySetOf(() => _graph.Matched)
        ...
        {
            return VerifySetOf(() => _graph.NoSubscribers)
        ...
        {
            return VerifySetOf(() => _graph.NoPublishers)
        ...
        {
            return VerifySetOf(() => _graph.Mismatches)
...
");
        ...
        {
            StoryTellerAssert.Fail(_graph.Mismatches.Any(),
                () => $"Detected publisher/subscriber mismatches{Environment.NewLine}" +
                      _graph.Mismatches.Select(x => x.ToString()).Join(Environment.NewLine));
            StoryTellerAssert.Fail(_graph.NoPublishers.Any(),
                () => $"Detected subscriptions with no publishers{Environment.NewLine}" +
                      _graph.NoPublishers.Select(x => x.ToString()).Join(Environment.NewLine));
            StoryTellerAssert.Fail(_graph.NoSubscribers.Any(),
                () => $"Detected published messages with no subscribers{Environment.NewLine}" +
                      _graph.NoSubscribers.Select(x => x.ToString()).Join(Environment.NewLine));
