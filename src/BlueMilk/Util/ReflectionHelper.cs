...
namespace BlueMilk.Util
{
    ...
    {
        internal static bool MeetsSpecialGenericConstraints(Type genericArgType, Type proposedSpecificType)
        {
            ...
            if ((constraints & GenericParameterAttributes.DefaultConstructorConstraint) != 0
                && proposedSpecificType.GetTypeInfo().GetConstructor(Type.EmptyTypes) == null)
            {
                return false;
            }
        ...
        internal static PropertyInfo GetProperty<TModel, T>(Expression<Func<TModel, T>> expression)
        {
            MemberExpression memberExpression = getMemberExpression(expression);
            return (PropertyInfo) memberExpression.Member;
        }
        ...
        internal static MemberExpression GetMemberExpression(this LambdaExpression expression, bool enforceMemberExpression)
        {
            MemberExpression memberExpression = null;
            if (expression.Body.NodeType == ExpressionType.Convert)
            {
        ...
        internal static bool IsMemberExpression<T>(Expression<Func<T, object>> expression)
        {
            return IsMemberExpression<T, object>(expression);
        }

        internal static bool IsMemberExpression<T, U>(Expression<Func<T, U>> expression)
        {
            return GetMemberExpression(expression, false) != null;
        }


        internal static MethodInfo GetMethod<T>(Expression<Func<T, object>> expression)
        {
            return new FindMethodVisitor(expression).Method;
        }

        internal static MethodInfo GetMethod(Expression<Func<object>> expression)
        {
            return GetMethod<Func<object>>(expression);
        }

        internal static MethodInfo GetMethod(Expression expression)
        {
            return new FindMethodVisitor(expression).Method;
        }

        internal static MethodInfo GetMethod<TDelegate>(Expression<TDelegate> expression)
        {
            return new FindMethodVisitor(expression).Method;
        }

        internal static MethodInfo GetMethod<T, U>(Expression<Func<T, U>> expression)
        {
            return new FindMethodVisitor(expression).Method;
        }

        internal static MethodInfo GetMethod<T, U, V>(Expression<Func<T, U, V>> expression)
        {
            return new FindMethodVisitor(expression).Method;
        }

        internal static MethodInfo GetMethod<T>(Expression<Action<T>> expression)
        {
            return new FindMethodVisitor(expression).Method;
        }
    ...
    internal class FindMethodVisitor : ExpressionVisitorBase
    {
        private MethodInfo _method;

        internal FindMethodVisitor(Expression expression)
        {
            Visit(expression);
        }
